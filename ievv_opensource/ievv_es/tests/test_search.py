...
class PersonSearch(ievv_es.Search):
    def query_name(self, name):
        return self.query('match', name=name)


class FancyPersonSearch(ievv_es.Search):
    def query_all(self, text):
        return self.query(ievv_es.Q('match', name=text) | ievv_es.Q('match', description=text))


class PersonDocType(ievv_es.DocType):
...
class TestSearch(test.TestCase):
    ...
    def test_doctype(self):
        ...
        search = ievv_es.Search().query('match', name='Peter')
        pprint(search.to_dict())
        result = search.execute()
        pprint(result)
    ...
    def test_get_error(self):
        ...
        person.save()
        self.es.indices.flush()
        with self.assertRaises(ievv_es.exceptions.NotFoundError):
